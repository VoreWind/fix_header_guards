...
#if DOCTEST_CLANG && \
    DOCTEST_CLANG_HAS_WARNING("-Wzero-as-null-pointer-constant")
...
// C4548 - expression before comma has no effect; expected expression with side
// - effect
// C4986 - exception specification does not match previous declaration
// C4350 - behavior change: 'member1' called instead of 'member2'
// C4668 - 'x' is not defined as a preprocessor macro, replacing with '0' for
// '#if/#elif'
...
// MSVC C++11 feature support table:
...
#define DOCTEST_CONFIG_WITH_DELETED_FUNCTIONS
#endif  // clang
#if DOCTEST_GCC >= DOCTEST_COMPILER(4, 5, 0) && \
    defined(__GXX_EXPERIMENTAL_CXX0X__)
#define DOCTEST_CONFIG_WITH_DELETED_FUNCTIONS
#endif  // GCC
#endif  // DOCTEST_CONFIG_WITH_DELETED_FUNCTIONS

#if defined(DOCTEST_CONFIG_NO_DELETED_FUNCTIONS) && \
    defined(DOCTEST_CONFIG_WITH_DELETED_FUNCTIONS)
...
#define DOCTEST_CONFIG_WITH_RVALUE_REFERENCES
#endif  // clang
#if DOCTEST_GCC >= DOCTEST_COMPILER(4, 3, 0) && \
    defined(__GXX_EXPERIMENTAL_CXX0X__)
#define DOCTEST_CONFIG_WITH_RVALUE_REFERENCES
#endif  // GCC
#endif  // DOCTEST_CONFIG_WITH_RVALUE_REFERENCES

#if defined(DOCTEST_CONFIG_NO_RVALUE_REFERENCES) && \
    defined(DOCTEST_CONFIG_WITH_RVALUE_REFERENCES)
...
#if defined(DOCTEST_CONFIG_NO_VARIADIC_MACROS) && \
    defined(DOCTEST_CONFIG_WITH_VARIADIC_MACROS)
...
#if defined(DOCTEST_CONFIG_NO_LONG_LONG) && \
    defined(DOCTEST_CONFIG_WITH_LONG_LONG)
...
// other stuff...
    ...
    defined(DOCTEST_CONFIG_WITH_LONG_LONG) ||         \
    defined(DOCTEST_CONFIG_WITH_DELETED_FUNCTIONS) || \
    defined(DOCTEST_CONFIG_WITH_NULLPTR) ||           \
    defined(DOCTEST_CONFIG_WITH_VARIADIC_MACROS) ||   \
    defined(DOCTEST_CONFIG_WITH_STATIC_ASSERT)
...
// not forward declaring ostream for libc++ because I had some problems (inline
// namespaces vs c++98)
// so the <iosfwd> header is used - also it is very light and doesn't drag a ton
// of stuff
#include <iosfwd>
...
// static assert macro - because of the c++98 support requires that the message
// is an
// identifier (no spaces and not a C string) - example without quotes:
// I_am_a_message
...
#define DOCTEST_STATIC_ASSERT(expression, message)                            \
  struct DOCTEST_CAT(__static_assertion_at_line_, __LINE__) {                 \
    doctest::detail::static_assert_impl::StaticAssertion<                     \
        ...
        DOCTEST_CAT(DOCTEST_CAT(DOCTEST_CAT(STATIC_ASSERTION_FAILED_AT_LINE_, \
                                            __LINE__),                        \
                                _),                                           \
                    message);                                                 \
  };                                                                          \
  typedef doctest::detail::static_assert_impl::StaticAssertionTest<           \
      static_cast<int>(                                                       \
...
// macro
// introduces an anonymous namespace in which getCurrentTestSuite gets
// overridden
...
// can hold strings with length
// of up to 23 chars on the stack before going on the heap - the last byte of
...
class DOCTEST_INTERFACE String {
 ...
 public:
  ...
  // GCC 4.9/5/6 report Wstrict-overflow when optimizations are ON and it got
  // inlined in the vector class somewhere...
  // see commit 574ef95f0cd379118be5011704664e4b5351f1e0 and build
  // https://travis-ci.org/onqtam/doctest/builds/230671611
...
DOCTEST_INTERFACE std::ostream& operator<<(std::ostream& stream,
                                           const String& in);
...
// to silence the warning "-Wzero-as-null-pointer-constant" only for gcc 5 for
// the Approx template ctor - pragmas don't work for it...
...
struct ForEachType<Typelist<Head, Tail>, Callable>
    : public ForEachType<Tail, Callable> {
  enum { value = 1 + ForEachType<Tail, Callable>::value };
...
template <typename T>
struct StringMaker
    : detail::StringMakerBase<detail::has_insertion_operator<T>::value> {};
...
DOCTEST_MSVC_SUPPRESS_WARNING_PUSH
// http://stackoverflow.com/questions/39479163 what's the difference between
// C4018 and C4389
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)  \
  ...
  DOCTEST_NOINLINE Result operator op(const DOCTEST_REF_WRAP(R) rhs) { \
    ...
    if (m_assert_type & assertType::is_false) res = !res;              \
...
struct Expression_lhs {
  ...
  explicit Expression_lhs(L in, assertType::Enum assert_type)
      : lhs(in), m_assert_type(assert_type) {}
  ...
  // precedence over the comparisons, but the
  // ExpressionDecomposer class uses the left shift operator to capture the left
...
struct ExpressionDecomposer {
  ...
  // on the operator precedence table)
  // but then there will be warnings from GCC about "-Wparentheses" and since
  ...
  template <typename L>
  Expression_lhs<const DOCTEST_REF_WRAP(L)> operator<<(const DOCTEST_REF_WRAP(L)
                                                           operand) {
...
DOCTEST_NOINLINE int fast_binary_assert(assertType::Enum assert_type,
  ...
  if (rb.log()) res |= assertAction::dbgbreak;
  ...
  // #########################################################################################
  // IF THE DEBUGGER BREAKS HERE - GO 1 LEVEL UP IN THE CALLSTACK TO SEE THE
  // FAILING ASSERTION
  // THIS IS THE EFFECT OF HAVING 'DOCTEST_CONFIG_SUPER_FAST_ASSERTS' DEFINED
  // #########################################################################################
  if (res & assertAction::dbgbreak) DOCTEST_BREAK_INTO_DEBUGGER();
...
// FIX FOR VISUAL STUDIO VERSIONS PRIOR TO 2015 - they failed to compile the
// call to operator<< with
// std::ostream passed as a reference noting that there is a use of an undefined
// type (which there isn't)
DOCTEST_INTERFACE void writeStringToStream(std::ostream* stream,
                                           const String& str);
...
DOCTEST_INTERFACE void toStream(std::ostream* stream,
...
class ContextBuilder {
  ...
  void build(std::ostream* stream) const {
    ...
    while (curr < numCaptures && curr < DOCTEST_CONFIG_NUM_CAPTURES_ON_STACK)
      reinterpret_cast<const ICapture*>(stackChunks[curr++].buf)
          ->toStream(stream);
    // iterate over list
    Node* curr_elem = head;
 ...
 public:
  ...
  DOCTEST_NOINLINE ContextBuilder& operator<<(T& in) {
    ...
    // cant construct
    // the object directly in the buffer using placement new - need the <new>
    // header...
...
class ContextScope : public IContextScope {
 ...
 public:
  ...
  DOCTEST_NOINLINE ~ContextScope() {
    if (!built) useContextIfExceptionOccurred(this);
    popFromContexts();
  }
...
struct expected_failures {
  ...
  void fill(detail::TestSuite& state) const {
    state.m_expected_failures = data;
  }
};
...
#define DOCTEST_HANDLE_BRACED_VA_ARGS(expr) \
  DOCTEST_STRIP_PARENS(DOCTEST_EXPAND_VA_ARGS expr)
...
#define DOCTEST_TEST_CASE(decorators)                                          \
  DOCTEST_CREATE_AND_REGISTER_FUNCTION(DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), \
                                       decorators)
...
#define DOCTEST_TEST_CASE_TEMPLATE(decorators, T, types) \
  DOCTEST_TEST_CASE_TEMPLATE_IMPL(decorators, T, types,  \
                                  DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
#endif  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE(decorators, T, id) \
  DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL(                    \
      decorators, T, id, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE(id, ...) \
  DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(          \
      id, (__VA_ARGS__), DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
#else  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE(id, types) \
  DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(            \
      id, types, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
#endif  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_ADD_AT_IMPL(type, file, line, mb, x)                       \
  do {                                                                     \
    doctest::detail::MessageBuilder mb(file, line,                         \
                                       doctest::detail::assertType::type); \
    mb << x;                                                               \
...
#define DOCTEST_ASSERT_IMPLEMENT_2(expr, assert_type)                     \
  ...
  DOCTEST_WRAP_IN_TRY(                                                    \
      _DOCTEST_RB.setResult(doctest::detail::ExpressionDecomposer(        \
                                doctest::detail::assertType::assert_type) \
...
#define DOCTEST_REQUIRE(...) \
  DOCTEST_ASSERT_IMPLEMENT_1((__VA_ARGS__), DT_REQUIRE)
#define DOCTEST_WARN_FALSE(...) \
  DOCTEST_ASSERT_IMPLEMENT_1((__VA_ARGS__), DT_WARN_FALSE)
#define DOCTEST_CHECK_FALSE(...) \
  DOCTEST_ASSERT_IMPLEMENT_1((__VA_ARGS__), DT_CHECK_FALSE)
#define DOCTEST_REQUIRE_FALSE(...) \
  DOCTEST_ASSERT_IMPLEMENT_1((__VA_ARGS__), DT_REQUIRE_FALSE)
#else  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_CHECK_FALSE(expr) \
  DOCTEST_ASSERT_IMPLEMENT_1(expr, DT_CHECK_FALSE)
#define DOCTEST_REQUIRE_FALSE(expr) \
  DOCTEST_ASSERT_IMPLEMENT_1(expr, DT_REQUIRE_FALSE)
#endif  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_REQUIRE_THROWS(expr) \
  DOCTEST_ASSERT_THROWS(expr, DT_REQUIRE_THROWS)
...
#define DOCTEST_CHECK_NOTHROW(expr) \
  DOCTEST_ASSERT_NOTHROW(expr, DT_CHECK_NOTHROW)
#define DOCTEST_REQUIRE_NOTHROW(expr) \
  DOCTEST_ASSERT_NOTHROW(expr, DT_REQUIRE_NOTHROW)
...
#define DOCTEST_UNARY_ASSERT(assert_type, expr)                        \
  do {                                                                 \
    doctest::detail::ResultBuilder _DOCTEST_RB(                        \
        ...
        DOCTEST_TOSTR(DOCTEST_HANDLE_BRACED_VA_ARGS(expr)));           \
    DOCTEST_WRAP_IN_TRY(                                               \
        _DOCTEST_RB.unary_assert(DOCTEST_HANDLE_BRACED_VA_ARGS(expr))) \
    DOCTEST_ASSERT_LOG_AND_REACT(_DOCTEST_RB);                         \
  } while ((void)0, 0)

#ifdef DOCTEST_CONFIG_WITH_VARIADIC_MACROS
#define DOCTEST_WARN_EQ(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_EQ, (__VA_ARGS__), eq)
#define DOCTEST_CHECK_EQ(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_EQ, (__VA_ARGS__), eq)
#define DOCTEST_REQUIRE_EQ(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_EQ, (__VA_ARGS__), eq)
#define DOCTEST_WARN_NE(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_NE, (__VA_ARGS__), ne)
#define DOCTEST_CHECK_NE(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_NE, (__VA_ARGS__), ne)
#define DOCTEST_REQUIRE_NE(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_NE, (__VA_ARGS__), ne)
#define DOCTEST_WARN_GT(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_GT, (__VA_ARGS__), gt)
#define DOCTEST_CHECK_GT(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_GT, (__VA_ARGS__), gt)
#define DOCTEST_REQUIRE_GT(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_GT, (__VA_ARGS__), gt)
#define DOCTEST_WARN_LT(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_LT, (__VA_ARGS__), lt)
#define DOCTEST_CHECK_LT(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_LT, (__VA_ARGS__), lt)
#define DOCTEST_REQUIRE_LT(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_LT, (__VA_ARGS__), lt)
#define DOCTEST_WARN_GE(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_GE, (__VA_ARGS__), ge)
#define DOCTEST_CHECK_GE(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_GE, (__VA_ARGS__), ge)
#define DOCTEST_REQUIRE_GE(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_GE, (__VA_ARGS__), ge)
#define DOCTEST_WARN_LE(...) \
  DOCTEST_BINARY_ASSERT(DT_WARN_LE, (__VA_ARGS__), le)
#define DOCTEST_CHECK_LE(...) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_LE, (__VA_ARGS__), le)
#define DOCTEST_REQUIRE_LE(...) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_LE, (__VA_ARGS__), le)
...
#else  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
#define DOCTEST_WARN_EQ(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_EQ, lhs, rhs, eq)
#define DOCTEST_CHECK_EQ(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_EQ, lhs, rhs, eq)
#define DOCTEST_REQUIRE_EQ(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_EQ, lhs, rhs, eq)
#define DOCTEST_WARN_NE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_NE, lhs, rhs, ne)
#define DOCTEST_CHECK_NE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_NE, lhs, rhs, ne)
#define DOCTEST_REQUIRE_NE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_NE, lhs, rhs, ne)
#define DOCTEST_WARN_GT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_GT, lhs, rhs, gt)
#define DOCTEST_CHECK_GT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_GT, lhs, rhs, gt)
#define DOCTEST_REQUIRE_GT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_GT, lhs, rhs, gt)
#define DOCTEST_WARN_LT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_LT, lhs, rhs, lt)
#define DOCTEST_CHECK_LT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_LT, lhs, rhs, lt)
#define DOCTEST_REQUIRE_LT(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_LT, lhs, rhs, lt)
#define DOCTEST_WARN_GE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_GE, lhs, rhs, ge)
#define DOCTEST_CHECK_GE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_GE, lhs, rhs, ge)
#define DOCTEST_REQUIRE_GE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_GE, lhs, rhs, ge)
#define DOCTEST_WARN_LE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_WARN_LE, lhs, rhs, le)
#define DOCTEST_CHECK_LE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_CHECK_LE, lhs, rhs, le)
#define DOCTEST_REQUIRE_LE(lhs, rhs) \
  DOCTEST_BINARY_ASSERT(DT_REQUIRE_LE, lhs, rhs, le)
...
#define DOCTEST_CHECK_UNARY_FALSE(v) \
  DOCTEST_UNARY_ASSERT(DT_CHECK_UNARY_FALSE, v)
#define DOCTEST_REQUIRE_UNARY_FALSE(v) \
  DOCTEST_UNARY_ASSERT(DT_REQUIRE_UNARY_FALSE, v)
#endif  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#ifdef DOCTEST_CONFIG_WITH_VARIADIC_MACROS
#define DOCTEST_FAST_BINARY_ASSERT(assert_type, expr, comparison)   \
  doctest::detail::fast_binary_assert<                              \
      doctest::detail::binaryAssertComparison::comparison>(         \
      doctest::detail::assertType::assert_type, __FILE__, __LINE__, \
      DOCTEST_TOSTR(DOCTEST_HANDLE_BRACED_VA_ARGS(expr)),           \
      DOCTEST_HANDLE_BRACED_VA_ARGS(expr))
...
#define DOCTEST_FAST_UNARY_ASSERT(assert_type, expr)                \
  doctest::detail::fast_unary_assert(                               \
      doctest::detail::assertType::assert_type, __FILE__, __LINE__, \
      DOCTEST_TOSTR(DOCTEST_HANDLE_BRACED_VA_ARGS(expr)),           \
      DOCTEST_HANDLE_BRACED_VA_ARGS(expr))
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                      \
  template <typename DOCTEST_UNUSED_TEMPLATE_TYPE>                            \
  static inline doctest::String DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR_)( \
      signature)
...
void my_memcpy(void* dest, const void* src, unsigned num) {
  ...
  for (unsigned i = 0; i < num; ++i) cdest[i] = csrc[i];
}

// not using std::strlen() because of valgrind errors when optimizations are
// turned on
// 'Invalid read of size 4' when the test suite len (with '\0') is not a
// multiple of 4
...
struct ContextState : TestAccessibleContextState  //! OCLINT too many fields
{
  ...
  std::vector<std::string>
      exceptionalContexts;  // logging from INFO() due to an exception
...
String& String::operator+=(const String& other) {
  ...
  if (isOnStack()) {
    ...
    if (data.capacity > total_size) {
      ...
      detail::my_memcpy(temp, data.ptr,
                        my_old_size);  // skip the +1 ('\0') for speed
      // release old chunk
      delete[] data.ptr;
...
int String::compare(const char* other, bool no_case) const {
  if (no_case) return detail::stricmp(c_str(), other);
...
bool operator==(double lhs, Approx const& rhs) {
  ...
  return std::fabs(lhs - rhs.m_value) <
         rhs.m_epsilon * (rhs.m_scale + detail::my_max(std::fabs(lhs),
                                                       std::fabs(rhs.m_value)));
}

String Approx::toString() const {
  return String("Approx( ") + doctest::toString(m_value) + " )";
}
...
bool checkIfShouldThrow(assertType::Enum assert_type) {
  ...
  if ((assert_type &
       assertType::is_check)  //! OCLINT bitwise operator in conditional
...
void fastAssertThrowIfFlagSet(int flags) {
  if (flags &
      assertAction::shouldthrow)  //! OCLINT bitwise operator in conditional
    throwException();
}
void throwException() {
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
...
bool matchesAny(const char* name,
  ...
  for (unsigned i = 0; i < filters.size(); ++i)
    if (wildcmp(name, filters[i].c_str(), caseSensitive)) return true;
  return false;
}
...
UInt64 getCurrentTicks() {
  timeval t;
  gettimeofday(&t, 0);
  return static_cast<UInt64>(t.tv_sec) * 1000000 +
...
Subcase::Subcase(const char* name, const char* file, int line)
  ...
  if (s->hasLoggedCurrentTestStart) logTestEnd();
...
Subcase::Subcase(const Subcase& other)
    : m_signature(other.m_signature.m_name,
                  other.m_signature.m_file,
...
Subcase::~Subcase() {
  if (m_entered) {
    ...
    if (s->subcasesHasSkipped == false) s->subcasesPassed.insert(m_signature);

    if (!s->subcasesStack.empty()) s->subcasesStack.pop_back();
    if (s->hasLoggedCurrentTestStart) logTestEnd();
    s->hasLoggedCurrentTestStart = false;
  }
}
...
void Color::init() {
  ...
  if (!g_attrsInitted) {
    ...
    g_originalForegroundAttributes = csbiInfo.wAttributes &
                                     ~(BACKGROUND_GREEN | BACKGROUND_RED |
                                       BACKGROUND_BLUE | BACKGROUND_INTENSITY);
    g_originalBackgroundAttributes = csbiInfo.wAttributes &
...
void Color::use(Code
  ...
  if (p->no_colors) return;
#ifdef DOCTEST_CONFIG_COLORS_ANSI
  if (isatty(STDOUT_FILENO) == false && p->force_colors == false) return;
...
#ifdef DOCTEST_CONFIG_COLORS_WINDOWS
  if (isatty(fileno(stdout)) == false && p->force_colors == false) return;
...
void registerExceptionTranslatorImpl(
    const IExceptionTranslator* translateFunction) {
...
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
  String res;
  std::vector<const IExceptionTranslator*>& translators =
      getExceptionTranslators();
  for (size_t i = 0; i < translators.size(); ++i)
    if (translators[i]->translate(res)) return res;
  // clang-format off
        try {
            throw;
...
void toStream(std::ostream* stream, int long long unsigned in) {
  *stream << in;
}
#endif  // DOCTEST_CONFIG_WITH_LONG_LONG

void addToContexts(IContextScope* ptr) {
  contextState->contexts.push_back(ptr);
}
...
int lineForOutput(int line) {
  if (contextState->no_line_numbers) return 0;
  return line;
}
...
#ifdef DOCTEST_PLATFORM_WINDOWS
void myOutputDebugString(const String& text) {
...
void printToDebugConsole(const String& text) {
  if (isDebuggerActive()) myOutputDebugString(text.c_str());
}
...
void logTestStart(const TestCase& tc) {
  ...
  if (std::string(tc.m_name).substr(0, DOCTEST_COUNTOF(scenario) - 1) ==
      scenario)
    n1[0] = '\0';
  ...
  printToDebugConsole(String(getSeparator()) + loc + forDebugConsole.c_str() +
                      n1 + n2 + subcaseStuff.c_str() + "\n");
}
...
String logContext() {
  ...
  if (!contexts.empty()) stream << "with context:\n";
...
const char* getFailString(assertType::Enum assert_type) {
  if (assert_type &
      assertType::is_warn)  //! OCLINT bitwise operator in conditional
    return "WARNING";
  if (assert_type &
      assertType::is_check)  //! OCLINT bitwise operator in conditional
    return "ERROR";
...
void logAssert(bool passed,
  ...
  DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
  DOCTEST_PRINTF_COLORED(
      msg, passed ? Color::BrightGreen : isWarn ? Color::Yellow : Color::Red);
...
void logAssertThrows(bool threw,
                     const char* expr,
                     assertType::Enum assert_type,
                     const char* file,
                     int line) {
  ...
  DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
  DOCTEST_PRINTF_COLORED(
      msg, threw ? Color::BrightGreen : isWarn ? Color::Yellow : Color::Red);
  DOCTEST_PRINTF_COLORED(info1, Color::Cyan);
  DOCTEST_PRINTF_COLORED(info2, Color::None);
  String context = logContext();
  DOCTEST_PRINTF_COLORED(context.c_str(), Color::None);
...
void logAssertThrowsAs(bool threw,
  ...
  if (!threw) {  //! OCLINT inverted logic
    ...
    DOCTEST_SNPRINTF(info2, DOCTEST_COUNTOF(info2),
  ...
  DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
  DOCTEST_PRINTF_COLORED(
      msg, threw_as ? Color::BrightGreen : isWarn ? Color::Yellow : Color::Red);
  DOCTEST_PRINTF_COLORED(info1, Color::Cyan);
  DOCTEST_PRINTF_COLORED(info2, Color::None);
  DOCTEST_PRINTF_COLORED(info3, Color::Cyan);
  String context = logContext();
...
void logAssertNothrow(bool threw,
  ...
  DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
  DOCTEST_PRINTF_COLORED(
      msg, threw ? isWarn ? Color::Yellow : Color::Red : Color::BrightGreen);
  DOCTEST_PRINTF_COLORED(info1, Color::Cyan);
  DOCTEST_PRINTF_COLORED(info2, Color::None);
  DOCTEST_PRINTF_COLORED(info3, Color::Cyan);
  String context = logContext();
...
bool ResultBuilder::log() {
  if ((m_assert_type & assertType::is_warn) ==
      0)  //! OCLINT bitwise operator in conditional
  ...
  if (m_failed || contextState->success) {
    ...
    if (m_assert_type &
      ...
      logAssertThrows(m_threw, m_expr, m_assert_type, m_file, m_line);
               ...
               assertType::is_throws_as) {
      logAssertThrowsAs(m_threw, m_threw_as, m_exception_type, m_exception,
                        m_expr, m_assert_type, m_file, m_line);
    } else if (m_assert_type &  //! OCLINT bitwise operator in conditional
               assertType::is_nothrow) {
      logAssertNothrow(m_threw, m_exception, m_expr, m_assert_type, m_file,
                       m_line);
    } else {
      logAssert(m_result.m_passed, m_result.m_decomposition.c_str(), m_threw,
                m_exception, m_expr, m_assert_type, m_file, m_line);
    }
  }

  if (m_failed) addFailedAssert(m_assert_type);

  return m_failed && isDebuggerActive() &&
         !contextState->no_breaks;  // break into debugger
}

void ResultBuilder::react() const {
  if (m_failed && checkIfShouldThrow(m_assert_type)) throwException();
}
...
bool MessageBuilder::log() {
  ...
  return isDebuggerActive() && !contextState->no_breaks &&
         !isWarn;  // break into debugger
}
...
bool parseCommaSepArgs(int argc,
  ...
  if (parseOption(argc, argv, pattern, filtersString)) {
    ...
    while (pch != 0) {
      if (my_strlen(pch)) res.push_back(pch);
...
void printSummary() {
  ...
  if (p->count || p->list_test_cases) {
    ...
    if (p->no_skipped_summary == false) {
      ...
      DOCTEST_PRINTF_COLORED(buff,
                             numSkipped == 0 ? Color::None : Color::Yellow);
    }
    DOCTEST_PRINTF_COLORED("\n", Color::None);

    DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);

    DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), "assertions: %6d",
                     p->numAssertions);
    DOCTEST_PRINTF_COLORED(buff, Color::None);
    DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), " | ");
    DOCTEST_PRINTF_COLORED(buff, Color::None);
    ...
    DOCTEST_PRINTF_COLORED(buff, (p->numAssertions == 0 || anythingFailed)
                                     ? Color::None
                                     : Color::Green);
    DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), " | ");
    DOCTEST_PRINTF_COLORED(buff, Color::None);
    DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), "%6d failed",
                     p->numFailedAssertions);
    DOCTEST_PRINTF_COLORED(
        buff, p->numFailedAssertions > 0 ? Color::Red : Color::None);
    ...
    DOCTEST_PRINTF_COLORED(result,
                           p->numFailed > 0 ? Color::Red : Color::Green);
  }
...
#define DOCTEST_PARSE_AS_BOOL_OR_FLAG(name, sname, var, default)          \
  if (parseIntOption(argc, argv, DOCTEST_STR_CONCAT_TOSTR(name, =),       \
                     option_bool, intRes) ||                              \
      parseIntOption(argc, argv, DOCTEST_STR_CONCAT_TOSTR(sname, =),      \
                     option_bool, intRes))                                \
    p->var = !!intRes;                                                    \
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)               \
  if (parseOption(argc, argv, DOCTEST_STR_CONCAT_TOSTR(name, =), strRes,  \
                  default) ||                                             \
      parseOption(argc, argv, DOCTEST_STR_CONCAT_TOSTR(sname, =), strRes, \
                  default) ||                                             \
      withDefaults)                                                       \
  p->var = strRes
...
#undef DOCTEST_PARSE_AS_BOOL_OR_FLAG
  ...
  if (parseFlag(argc, argv, "dt-list-test-cases") ||
      parseFlag(argc, argv, "dt-ltc")) {
    p->list_test_cases = true;
    p->exit = true;
  }
  if (parseFlag(argc, argv, "dt-list-test-suites") ||
      parseFlag(argc, argv, "dt-lts")) {
...
void Context::clearFilters() {
  for (unsigned i = 0; i < p->filters.size(); ++i) p->filters[i].clear();
}
...
int Context::run() {
  ...
  for (std::set<TestCase>::iterator it = registeredTests.begin();
       it != registeredTests.end(); ++it)
    testArray.push_back(&(*it));

  // sort the collected records
  if (!testArray.empty()) {
    if (p->order_by.compare("file", true) == 0) {
      std::qsort(&testArray[0], testArray.size(), sizeof(TestCase*),
                 fileOrderComparator);
  ...
  for (i = 0; i < testArray.size(); i++) {
    ...
    if (data.m_skip && !p->no_skip) continue;

    if (!matchesAny(data.m_file, p->filters[0], 1, p->case_sensitive)) continue;
    if (matchesAny(data.m_file, p->filters[1], 0, p->case_sensitive)) continue;
    ...
    if (matchesAny(data.m_test_suite, p->filters[3], 0, p->case_sensitive))
      continue;
    if (!matchesAny(data.m_name, p->filters[4], 1, p->case_sensitive)) continue;
    if (matchesAny(data.m_name, p->filters[5], 0, p->case_sensitive)) continue;
    ...
    // execute the test if it passes all the filtering
    {
      ...
      do {
        ...
        if (p->hasLoggedCurrentTestStart) logTestEnd();
        p->hasLoggedCurrentTestStart = false;

        // if logging successful tests - force the start log
        if (p->success) DOCTEST_LOG_START();
...
// execute the test
        ...
        try {
          ...
          data.m_test();
          fatalConditionHandler.reset();
          if (contextState->hasCurrentTestFailed) failed = true;
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
      ...
      if (data.m_should_fail) {
        ...
        DOCTEST_LOG_START();
        char msg[DOCTEST_SNPRINTF_BUFFER_LENGTH];
        if (p->numFailedAssertionsForCurrentTestcase ==
            data.m_expected_failures) {
          failed = false;
          DOCTEST_SNPRINTF(msg, DOCTEST_COUNTOF(msg),
                           "Failed exactly %d times as expected so marking it "
                           "as not failed!\n",
                           data.m_expected_failures);
      ...
      if (p->abort_after > 0 && p->numFailedAssertions >= p->abort_after) break;
    }
  }
  ...
  if (p->numFailed && !p->no_exitcode) return EXIT_FAILURE;
  return EXIT_SUCCESS;
}
}  // namespace doctest
